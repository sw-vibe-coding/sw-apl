...
| Libraries by mode: `)LIB`, `)LOAD`, `)SAVE`, `)DROP`, `)COPY` see only the workspaces that run in the mode (`--mode 70` or `75`) | done | done | shelves tests, session modes tests |
...
| (B) '75: a system variable localized in a function header, `⎕PW` 128 while a definition is open, indexed assignment into a system variable | -- | not implemented | NONCE ERROR, or not implemented |
