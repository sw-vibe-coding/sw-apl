...
feature, with its status in each mode and what pins it. sw-apl has
two modes: (A) '70, modelled on APL\360, and (B) '75, modelled on the
APL of the IBM 5100 family (`mode-b.md`). Each table has a column per
mode. "done" means implemented and locked by a unit test or a reg-rs
...
does not do and will not -- is not a row at all: it is under
**Restrictions** at the foot of this file, with the decision it
follows from.

There are **no** `todo` cells, in either column: everything is
`done` or `part`, and everything sw-apl will not do is a restriction
with a reason. The check, matching table cells and not this
sentence: `grep -c '^|.*| todo |' docs/parity.md` prints 0.

## How we will know we have parity

For (A) '70: we have no running APL\360 to compare against, so the
oracle is the IBM APL\360 User's Manual (1968, and its 1970 edition), with the
APL\360-OS/DOS manual of December 1970 for domino,
and the criterion is:

1. Every row below is `done` in the (A) column.
...
For (B) '75 the oracle is the IBM 5110 APL Reference Manual, with
the 5100's and the APLSV manuals where it defers to them
(`mode-b.md` lists them, and what (B) adds, drops and changes). The
criterion is the same shape: every row `done` in the (B) column, and
its samples (76, 77, 78) reproducing the manual's behaviour.

## Syntax and the session

| Feature | (A) '70 | (B) '75 | Pinned by |
| --- | --- | --- | --- |
| Numeric literals, high minus, E notation | done | done | lex tests, samples 01, 03 |
| Strands, names, lamp comments, parentheses | done | done | lex/parse tests, sample 21 |
| Right-to-left evaluation, long right scope | done | done | parse/eval tests, sample 01 |
...
| Bracket indexing `A[I;J]`, elided axes, index arrays of any rank | done | done | index tests, sample 48 |
| Indexed assignment with scalar extension | done | done | index tests, sample 48 |
| Branch `→`, labels as local constants, `→0`, branch off the end, empty branch, the `→LABEL×⍳COND` idiom | done | done | scan/eval/session tests, samples 50, 54 |
| Defined functions: del headers (`NAME`, `NAME B`, `A NAME B`, each with or without `R←`), locals, calls, the result at exit | done | done | eval/session tests, sample 53 |
| Definition mode: an opening `∇` collects body lines behind the `[n]` prompt until a closing `∇` | done | done | session/cli tests, sample 53 |
| Del editor: `[n]`, `[⎕]`, `[n⎕]`, `[∆n]`, fractional insert and renumber on close, header edit and rename `[0]`, reopen `∇NAME`, lock `⍫` to open or to close, either being enough, and `⍫NAME` reopening an unlocked function to lock it | done | done | session tests, locked tests, sample 55 |
| Dynamic scoping, recursion | done | done | eval tests, samples 53, 54. A recursion with no branch to stop it reaches DEPTH ERROR |
| A local name hides a global function as well as a global variable, and gives it back on return; under a suspension `)FNS` and `)VARS` list the globals and `)SAVE` writes them | done | done | workspace and local names session tests, sample 79 |
| State indicator: suspension, pendent callers, resumption with `→`n, clearing with a bare `→` | done | done | call/session tests, sample 56 |
| Six-space prompt, batch echo | done | done | cli tests |
| `aplterm`, a 2741 terminal: key translation, overstrikes formed at the keyboard, circled display of underscored letters, composed Unicode lines on the wire | done | done | `components/term`: keyboard tests, display tests, and a PTY test that drives the real binary and checks what it shows and what it sends |
| The 2741 keyboard in a browser: the same keymap, the same overstrikes, the same cells, from the same crate compiled to WebAssembly | done | done | `components/web/crates/apl-board`: what a keystroke means is tested natively; the page itself is checked by driving a browser, where `"wsid` types `)WSID` and `○` Ctrl-`]` `*` composes `⍟` |
| The board per mode: (B) shares the 2741 layout, which the 5100 family kept, and adds a CMD mode of the 5110's key-front legends; each mode's Commands list holds only its commands | done | done | board tests, check-pages |
| A session with no service at all: the interpreter compiled to WebAssembly, running on a Web Worker, with `⎕`, `⍞` and the del editor reading through a shared channel | done | done | `components/web/crates/apl-wasm`: the channel layout is tested natively; the session itself is checked by driving the built page in a browser, since a worker and a `SharedArrayBuffer` have no native equivalent. `)SAVE` and `)LOAD` have no filesystem under them and report so |
| A local service: one session per connection, each with its own workspace, over a socket for `aplterm` and over a WebSocket for a browser. `⎕`, `⍞` and the del editor read from the terminal mid-statement | done | done | `components/web`: link, socket and browser tests, the last driving the real binary over HTTP and a WebSocket. Remote interruption (ATTN while a statement runs) is not implemented |
| Executable `.apl` file: a leading `#!` line is the shell's and is not read as APL | done | done | reg-rs CLI tests, `tests/scripts/` |
| `)OFF` sign-off: time and date, connect time, processor time | done | done | session tests, sample 59 |
| Line editing with history (up arrow), Ctrl-C cancels a line, Ctrl-D signs off | done | done | manual (rustyline); tape |
| Interrupt a running statement: INTERRUPT, suspended, state indicator kept | done | done | call tests; a body is read between its lines, so one long line cannot yet be stopped |
| Error display: name, statement, caret | done | done | session tests |
| Error display inside functions: `FN[n]` heading the statement, caret aligned under it | done | done | session tests, sample 56 |
...
| The axis in brackets may be a one-element array: the notes to the manual's Table 3.8 let one replace any scalar | done | done | join tests, sample 74 |
| Structural functions on scalars and empty arrays | done | done | session audit test |
| Mixed output `'TEXT';X;'MORE'` (semicolon list) | done | done | session tests, sample 46 |
| Invalid UTF-8 reported with byte offset, run continues | done | done | cli tests |

## Scalar functions

| Glyph | (A) '70 | (B) '75 | Notes |
| --- | --- | --- | --- |
| `+` | monadic, dyadic | monadic, dyadic |  |
| `-` | monadic, dyadic | monadic, dyadic |  |
| `×` | monadic, dyadic | monadic, dyadic |  |
| `÷` | monadic, dyadic | monadic, dyadic | 0÷0 is 1 |
| `⌈` | monadic, dyadic | monadic, dyadic | tolerant |
| `⌊` | monadic, dyadic | monadic, dyadic | tolerant |
| `\|` | monadic, dyadic | monadic, dyadic | tolerant residue |
| `*` | monadic, dyadic | monadic, dyadic |  |
| `⍟` | monadic, dyadic | monadic, dyadic | sample 47 |
| `○` | monadic, dyadic | monadic, dyadic | pi times; k from ¯7 to 7 |
| `!` | monadic, dyadic | monadic, dyadic | exact to 20, gamma beyond |
| `~` | monadic | monadic | 0 and 1 only |
| `∧ ∨ ⍲ ⍱` | dyadic | dyadic | 0 and 1 only |
| `< ≤ = ≥ > ≠` | dyadic | dyadic | with fuzz; `=` and `≠` compare characters too (APL\360 User's Manual p. 3.8), element by element, in reduction and in inner and outer products; a character never equals a number; the other four take numbers only; a scan of characters is DOMAIN ERROR, since its result would mix characters and numbers; samples 15, 47, 75 |
| `?` | monadic, dyadic | monadic, dyadic | roll and deal via the random link (starts at 16807); deal takes a scalar or a one-element vector for each argument, so `A[(⍴A)?⍴A]` shuffles; samples 41, 47 |
...
| Glyph | (A) '70 | (B) '75 | Notes |
| --- | --- | --- | --- |
| `⍳` | monadic, dyadic | monadic, dyadic | index of, one past the end when absent; the index generator takes a scalar or a one-element vector, so `⍳⍴V` works; samples 30, 73 |
| `⍴` | monadic, dyadic | monadic, dyadic |  |
| `,` | monadic, dyadic | monadic, dyadic | any rank, axis bracket, laminate, scalar and rank-1 conformance; sample 49. Monadic ravel with an axis is APL2, not implemented |
| `⌽ ⊖` | monadic, dyadic | monadic, dyadic | axis bracket; vector shifts; a one-element left argument rotates as a scalar does; samples 08, 34 |
| `⍉` | monadic, dyadic | monadic, dyadic | dyadic permutes and takes diagonals; sample 35 |
...
| Form | (A) '70 | (B) '75 | Notes |
| --- | --- | --- | --- |
| Reduce `f/` last axis | done | done | scalar dyadic f; identity table for empties |
| Reduce first axis `f⌿`, axis `f/[k]` | done | done | ops tests, sample 52 |
| Scan `f\`, `f⍀` | done | done | prefix reductions on either axis; samples 33, 52 |
| Inner product `f.g` | done | done | any ranks, scalar extension; sample 39 |
| Outer product `∘.f` | done | done | result shape is both shapes catenated; sample 38 |

## Display

| Feature | (A) '70 | (B) '75 | Notes |
| --- | --- | --- | --- |
| Integers, high minus, single-space vectors | done | done |  |
| Numbers to `)DIGITS` significant digits, E form (integers too) | done | done, to `⎕PP`; a whole number of up to ten digits in full |  |
| Matrix columns right-aligned | done | done |  |
| Empty vector prints a blank line | done | done |  |
| Rank 3 and higher (blank lines between planes) | done | done |  |
| Character arrays without quotes | done | done | display tests, samples 19 and 50 |
| `)WIDTH` wrapping (vectors between elements, matrices in column blocks) | done | done, at `⎕PW` |  |
| Mixed integer and float columns (each element formatted, right-aligned) | done | done |  |

## Workspaces

| Feature | (A) '70 | (B) '75 | Pinned by |
| --- | --- | --- | --- |
| The workspace as a value: what `)SAVE` writes is separate from the terminal it runs on | done | done | workspace tests |
| `)CLEAR` clears the state indicator with everything else | done | done | session tests, sample 60 |
| Workspace file: re-executable UTF-8, stable byte-for-byte, round trips through the interpreter | done | done | wsfile tests, `tests/scripts/saved-workspace.apl.ws` |
| Workspace file names the modes it runs in (`⍝!MODES`) and carries its settings and random link as directives both modes read | done | done | wsfile modes tests, session modes tests |
| Libraries by mode: `)LIB`, `)LOAD`, `)SAVE`, `)DROP`, `)COPY` see only the workspaces that run in the mode (`--mode 70` or `75`); a file is named for its modes, `NAME.apl.ws`, `NAME.a-70.apl.ws` or `NAME.b-75.apl.ws`, and a workspace that differs by mode, as BIRDS does, has one of each | done | done | shelves tests, session modes and library tests, check-provenance |
| '70-only parts in their own crates (`components/a70/`), reached only in (A): the I-beams, `)ORIGIN` `)DIGITS` `)WIDTH`, `)GROUP` `)GRP` `)GRPS`. In (B) an I-beam is NONCE ERROR and those commands INCORRECT COMMAND | done | done | a70 crate tests, session modes tests |
...
| (B) '75: format `⍕`, in `components/b75/`: monadic, the display as characters; dyadic, each number in a field of a given width, in decimal form to a number of places or scaled form to a number of digits, a pair for every column or one for each, width 0 the narrowest that leaves a space; a number too wide for its field is DOMAIN ERROR | -- | done | format session tests (the manual's examples), sample 82 |
...
| (B) '75: a function fixed under a name that is running, waiting, locked or a variable is refused, the header being the line at fault; one fixed under a name a running function has made local is local to that call, and `⎕EX` erases the local only | -- | done | system functions and local names session tests, sample 78 |
...
| Command | (A) '70 | (B) '75 | Notes |
| --- | --- | --- | --- |
| Only the first four characters of a command name are significant | done | done | `)CLEA`, `)CLEAR` and `)CLEAVER` are one command, and a name of four characters or fewer must be exact. Name tests, sample 68 |
| `)OFF` | done | done |  |
| `)ORIGIN` `)DIGITS` `)WIDTH` | done | INCORRECT COMMAND | `)WIDTH` accepts 30 to 254 where the manual's table says 30 to 130. Deliberate: a 360 printed on a terminal 130 columns wide at most, and a window today is wider. The lower bound and the `WAS n` reply are the manual's |
| `)CLEAR` | done | done |  |
| `)WSID` | done | done | a workspace name must be an APL name, which `)WSID` holds to as `)SAVE` does |
| A workspace name is an APL name | done | done | it becomes a filename, so `)SAVE A/B`, `)LOAD ../X` and the `NAME:LOCK` password form are INCORRECT COMMAND. Lower case is accepted, as it is for every other name here and was not in APL\360. Library name tests, sample 65 |
| `)SAVE` `)LOAD` `)DROP` `)LIB` `)COPY` `)PCOPY` `)CONTINUE` | done | done | `)DROP` replies with the moment alone, as APL\360's did |
| `)FNS` `)VARS` `)ERASE` | done | done |  |
...
| `)HELP`, an sw-apl command: the mode's commands by group, and a page per command and topic from `data/help.txt`, each only in the modes that have it | done | done | extension tests, samples 83 and 84 |
| Library form `)LOAD 1 NAME`, DESCRIBE convention | done | done | `--library` sets the directory they are under, library 1 is `ws/lib1/`, and LEARN, LIFE, RACE, EDIT and BIRDS each carry a DESCRIBE, and so does TTTML, (B)'s alone; library tests, samples 64, 72, 80, 81 and 87. A library is somewhere workspaces are kept, not a directory: the browser keeps library 1 in the bundle and library 0 in its own storage, and the commands do not change. Store tests |
...
| `)COPY` and `)PCOPY` print the SAVED line, and `)PCOPY` a NOT COPIED list | done | done | the manual's WC3 and WC4: "SAVED, followed by the time of day and the date that the source workspace was last stored", and "NOT COPIED:, followed by the names of objects not copied". Command tests, samples 61 and 65 |
| `)NUMBER` `)OFF HOLD` `)CONTINUE HOLD` `)MSG` `)MSGN` `)OPR` `)OPRN` `)PORTS`, and the `[LOCK]`/`[KEY]` passwords | not implemented | not implemented | they answer INCORRECT COMMAND. They are the multi-user surface of a shared machine -- accounts at ports, an operator, other users to message, a library someone else can read -- and sw-apl has one user. `commands-reference.md` says so per command |

## Errors

| Error | (A) '70 | (B) '75 | Text pinned |
| --- | --- | --- | --- |
| SYNTAX, VALUE, DOMAIN, RANK, LENGTH | done | done | done |
| CHARACTER (with code point) | done | done | done |
| INDEX, DEFN, DEPTH | done | done | done |
| INTERRUPT when a read finds no more input, and from the keyboard | done | done | eval/cli/call tests |
| WS FULL | done | done | done; the workspace holds a fixed number of bytes (`--ws-size`, default 1048576) and anything that will not fit is refused without changing it; space, workspace and session tests, sample 62 |
| A glyph used where it has no such form (`1~0`, `1⍋2`, `⍳/1 2`, `,[1]M`) | done | done | done; SYNTAX ERROR, because APL\360 has no such function and the sentence does not parse. A glyph that *has* the form and was given a bad argument is still DOMAIN ERROR |
| NOT IMPLEMENTED (temporary, must reach zero) | reached zero | reached zero | the error kind is gone from the vocabulary |
| NONCE ERROR: an I-beam, and what (B) has not implemented | -- | done | a70 crate tests, session modes tests, system variables session tests |
