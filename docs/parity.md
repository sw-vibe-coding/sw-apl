...
(A) has **no** `todo` cells: everything in its column is `done`, and
everything sw-apl will not do is a restriction with a reason. (B)
has one, format. The check, matching table cells and not this
sentence: `grep -c '^|.*| todo |' docs/parity.md` prints 1.
...
| (B) '75: format `⍕` | -- | todo | NONCE ERROR in (B) until it is implemented |
